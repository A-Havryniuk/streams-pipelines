...
public class FuckingPrint {
    ...
    public StringBuilder buildStudentRows(StringBuilder builder) {
        ...
        builder.append(students.stream()
                .map(s -> {
                    return String.format(pattern, s) + SEPARATOR +
                            taskNames.stream()
                                    .map(t -> String.format("%" + t.length() + "s", findTaskResult(s, t)))
                                    .collect(Collectors.joining(SEPARATOR)) +
                            SEPARATOR +
                            String.format("%.2f", getTotalOfStudent(s, totalScores)).replace(",", ".") +
                            SEPARATOR +
                            "   " + getMarkOfStudent(s, marks) + END;
                })
    ...
    private int getLengthName (List<String> students) {
        return Math.max(widhtFirstColumn, students.stream().mapToInt(String::length)
